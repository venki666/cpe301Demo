/*
 * TB6612­FNG_Motor_Control.c
 *
 * Created: 1/15/2020 7:51:51 PM
...
int main(void) 
{
  ...
  // Read ADCH value (left justified) for OCR0A
  OCR0A = 30;
  /* ready start value */
  revTickAvg = 0;
  Flag = 0; // Use for
  ...
  {
    Period = revTickAvg;
    // Calculate Speed/Rpm/No. of Revolutions

    // send Speed value to LCD or USART
    USART_SendString("Tick;Period;Frequency  ");
    snprintf(outs, sizeof(outs), "%f ", (float)Period); // print it
