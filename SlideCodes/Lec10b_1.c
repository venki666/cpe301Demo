...
#define I_memory 40 // number of samples kept for tik value
// capture Flag
volatile uint8_t Flag;
volatile uint8_t Direction = 0;
#define bit_is_clear(sfr, bit) (!(_SFR_BYTE(sfr) & _BV(bit)))
volatile uint32_t revTickArr[I_memory], revTickAvg;
...
ISR(TIMER1_CAPT_vect) {
  if (revCtr < I_memory) {
    ...
    revTickArr[revCtr] = (uint32_t)(tickv) + ((uint32_t)T1Ovs2 * 0x10000L);
  } else {
    for (int i = 0; i < I_memory; i++)
      ticks += revTickArr[i];
    revTickAvg = ticks / I_memory;
    Flag = 1;
    ticks = 0; revCtr=-1;
  }
...
int main(void) {
  ...
  while (1) {
    if (revTickAvg > 0 &&
        Flag == 1) // You're not Chuck Norris, DO NOT DIVIDE BY ZERO.
    {
      Period = (float)revTickAvg;
      Speed = (float)(Period * 120 * 0.0625 / 1000000UL );
      RPM = 60 / Speed;
      // send Speed value to LCD or USART
      USART_SendString("Tick;Period;Frequency  ");
      snprintf(outs, sizeof(outs), "%f ; %f s; %f rpm ", (float)Period, Speed,
               RPM); // print it
